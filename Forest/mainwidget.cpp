...
void MainWidget::Show_pic_human(int num, int type) //слот - вывести изображение человека (1 - лесника или 2 - браконьера)
{
    QLabel **new_img=nullptr; //указатель на новый label с картинкой человека
    ...
    int x_human, y_human; //координаты x, y человека
    int x_png, y_png, w_png, h_png; //координаты x, y изображения человека и его ширина, высота

    if(type==1) // лесник
    {
        x_human=forester.get_ox(); //получить значение поля человека
        y_human=forester.get_oy(); //получить значение поля человека
    }
    if(type==2) //браконьер
    {
        x_human=invaders.get_ox(num); //получить значение поля человека
        y_human=invaders.get_oy(num); //получить значение поля человека
    }

    x_png = x_human - RADIUS_PIC_HUMAN; //получить координату x верхнего левого угла изображения человека
    y_png = y_human - RADIUS_PIC_HUMAN; //получить координату y верхнего левого угла изображения человека
    w_png = h_png = RADIUS_PIC_HUMAN*2; // получить ширину и высоту изображения

    (*new_img) = new QLabel(" ", ui->place); //создать новый label для картинки данного человека
    (*new_img) ->setGeometry(x_png,y_png,w_png,h_png); //указать размеры картинки и её положение
    switch (type)
    {
    case 1:
...
void MainWidget::Show_pic_house() //слот - вывести изображение домика лесника
{
