...
struct plant_sort //структура для сортировки растений (их упорядочивание по высоте или агрессивности)
{
    int num; //номер растения в соотв. массиве объекта Forest
    int type; //номер растения в соотв. массиве объекта Forest
    int radius; //радиус этого растения
    int height; //высота этого растения
    int agressiv; //агрессивность этого растения
    int f_die=0; //флаг того, что растение завяло (оно уже не учитывается и будет удалено)
};
...
void MainWidget::Timer_tick() //слот интервала таймера для растений и погоды
{
    ...
    for(int i=0, kolvo=forest.get_kolvo_dirt(), rainf=weather.get_rainfall_tick(); i<kolvo; i++) //перебрать ячейки почвы
    {
        ...
        {
            forest.set_wet_dirt(i, (forest.get_wet_dirt(i)-RAND(DRY_TICK_SUN_MIN,DRY_TICK_SUN_MAX))); //влага испаряется с почвы
            if(forest.get_wet_dirt(i)<0) //если в почве не осталось влаги
            {
                forest.set_wet_dirt(i, 0); //обнулить влагу в почве (иначе отрицательна)
            }
        }
    ...
    int all_plants; //кол-во растений всех видов

    Init_plant_sort(&plants, &all_plants); //заполнить структуру сортировки и получить кол-во всех растений
    ...
    for(int i=0; i<all_plants; i++) //перебрать все растения, получающие питание
    {
        ...
        {
            if(plants[k].height>plants[i].height && plants[k].f_die!=1) //если высота текущего растения выше исходного и оно живое
            {
        ...
        sun_received = double(sun) * (1-(square_sun_shielded/square)); //растение получает солн. свет с учётом закрытой площади
        if(sun_received!=0) //если растение получило хоть немного солн. света
        {
            ...
            double percent_receipt = sun_received/double(sun); //процент полученного растения света от начального значения (солнце не в тени)
        ...
        {
            double viability = forest.get_viability_plant(num, type); //жизнеспособность растения
            forest.set_viability_plant(num, type, viability-0.3); //уменьшить жизнеспособность растения, так как без света оно не может
        ...
        if(f_died==1) //если растение умирает без света
        {
            ...
            plants[i].f_die=1; //отметить флагом удаление этого растения (оно больше не учитывается при других расчётах)
        }
        else //если растение не зачахло
        {
            ...
            else //растение получило недостаточно питания
            {
                ...
                if(forest.get_fertility_now_dirt(num_dirt)<0) //если в почве нет столько питания, сколько нужно растению
                {
                    ...
                    double viability = forest.get_viability_plant(num, type); //жизнеспособность растения
                    forest.set_viability_plant(num, type, viability-0.2); //уменьшить жизнеспособность растения
                    if(forest.get_viability_plant(num, type)<=0)
                    {
                        f_died=1; //отметить флагом, если растение совсем зачахло (оно погибнет)
                    }
    ...
    int f_repeat=1; //флаг повторения поиска завядших растений
    while(f_repeat==1) //искать завядшие растения и удалить их, пока не будут удалены все нужные
    {
        f_repeat=0; //повторять поиск больше не нужно
        for(int i=0; i<all_plants; i++) //перебрать все растения
        {
            if(plants[i].f_die==1) //если растение в структуре сортировки нужно удалить
            {
                f_repeat=1; //повторно искать

                QLabel **img_delete=nullptr; //указатель на label с картинкой растения
                forest.get_img(&img_delete, plants[i].num, plants[i].type); //получить адрес для объекта с изображением
                (*img_delete)->deleteLater(); //удалить qlabel с изображением растения
...
void MainWidget::Init_plant_sort(struct plant_sort **plant_sort, int *kolvo_all_plants) //получить значения растений и заполнить структуру сортировки
{
    ...
    for(int type=1, j=0; type<=3; type++) //заполнить структуру информацией о всех растениях
    {
        ...
        {
            (*plant_sort)[j].num=i;
            (*plant_sort)[j].type=type;
            (*plant_sort)[j].radius=forest.get_radius_plant(i, type);
            (*plant_sort)[j].height=forest.get_height_plant(i, type);
            (*plant_sort)[j].agressiv=forest.get_aggresiv_plant(i, type);
            j++;
        }
    }
}
