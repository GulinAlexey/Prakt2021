...
void MainWidget::Find_target(int num, int type) //найти цель для человека (тип: 1 - лесник, 2 - браконьер)
{
    ...
    case 1: //лесник
        if(weather.get_f_status()!=F_RAIN) //если сейчас не дождь
        {
            ...
            {
                for(int type=1; type<=3; type++) //перебрать все типы растений
                {
                    //for(int )
                }
            }
    ...
    case 2: //браконьер
        ...
        for(int i=0, kolvo=forest.get_kolvo_tree(); i<kolvo; i++)
        {
            ...
            int ox2 = forest.get_x_plant(i, TYPE_TREE); // координата ox текущего дерева
            int oy2 = forest.get_y_plant(i, TYPE_TREE); // координата oy текущего дерева
            int dist = sqrt(((ox2-ox1)*(ox2-ox1))+((oy2-oy1)*(oy2-oy1))); //расстояние между двумя точками

            if(dist<=forester.get_view_radius()) //если дерево находится внутри области видимости
            {
